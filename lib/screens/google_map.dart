    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError || snapshot.data == null) {
          ...
          // Return a basic map without reports if there's an error
          return Stack(
            children: [
              GoogleMap(
                initialCameraPosition: CameraPosition(
                  target: _currentLocation ?? LatLng(-8.9036, 13.2489),
                  zoom: 15.0,
                ),
        ...
        for (final report1 in reports) {
          ...
          if (lat1 != null && lon1 != null) {
            ...
            if (reportsInRadius.length >= 3) {
              double opacity = 0.0;
              if (reportsInRadius.length >= 3 && reportsInRadius.length < 6) {
                opacity = 0.3;
        ...
        if (!_initialLoadDone && widget.currentLocation != null) {
          for (final circle in heatmapCircles) {
            final distanceToCircleCenter = const latlong.Distance().distance(
              latlong.LatLng(widget.currentLocation!.latitude,
                  widget.currentLocation!.longitude),
              latlong.LatLng(circle.center.latitude, circle.center.longitude),
            );
